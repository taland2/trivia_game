...
| `v1_startRound` | `{matchId, categoryId?}` | `{roundIx, category, servings[3]}` | `categoryId` required iff mode=pick & player's pick-turn; mode=spin → server picks, response includes `spinResult` for the wheel animation (outcome decided server-side, animation is theater) |
